...
namespace Manager
{
    public class ColumnIniData
    {
        public ColumnIniData(int displayIndex, int width, string dataPropertyName)
        {
    ...
    public class ViewIniData
    {
        public string Name;
        public List<ColumnIniData> Columns = new List<ColumnIniData>();
    }
    public partial class BaseView : UserControl
    {
        ...
        public BaseView()
        {
            ...
            this.HandleDestroyed += new EventHandler(BaseView_HandleDestroyed);
        }
        ...
        void BaseView_HandleDestroyed(object sender, EventArgs e)
        {
            if ( dgView.Columns.Count < 1 )
                return;
            try
            {
                string s = GetType().Name;

                ViewIniData data = null;
                if ( Properties.Settings.Default.ViewData == null )
                    Properties.Settings.Default.ViewData = new System.Collections.ArrayList();
                else
                    foreach(ViewIniData v in Properties.Settings.Default.ViewData)
                        if ( v.Name == s )
                        {
                            data = v;
                            break;
                ...
                if ( data == null )
                {
                    data = new ViewIniData();
                    data.Name = GetType().Name;
                    Properties.Settings.Default.ViewData.Add(data);
                }
                data.Columns.Clear();
                foreach(DataGridViewColumn col in dgView.Columns)
                    data.Columns.Add(new ColumnIniData(col.DisplayIndex, col.Width, col.DataPropertyName));
...
#region UI handlers
        private void BaseView_Load(object sender, EventArgs e)
        {
            if ( !DesignMode )
            {
                ...
                System.Configuration.Configuration cfg = System.Configuration.ConfigurationManager.OpenExeConfiguration(System.Configuration.ConfigurationUserLevel.PerUserRoamingAndLocal); 

                if ( Properties.Settings.Default.ViewData == null || Properties.Settings.Default.ViewData.Count < 1 )
                    return;
                ...
                if ( data == null || data.Columns == null || data.Columns.Count < 1 )
                    return;

                foreach(DataGridViewColumn col in dgView.Columns)
                    foreach(ColumnIniData ini in data.Columns)
...
#region virtual methods to be overriden in derived views - where will perform specific behaviour
        ...
        public virtual void OnRefresh()
        {
            ...
            try
            {
                ...
                {
                    dataAdapter = new SqlDataAdapter(selectCommand, Program.CfgData.Sql.GetConnStr(true));
        ...
        void tbl_RowChanged(object sender, DataRowChangeEventArgs e)
        {
            if ( (dataAdapter != null) && (bindDataSrc.DataSource is DataTable) && 
                (e.Action == DataRowAction.Add || e.Action == DataRowAction.Change || e.Action == DataRowAction.Delete) )
            {
                Cursor = Cursors.WaitCursor;
                try
                {
                ...
                {
                    string s = string.Format("Error occured when saving changes to database!{0}{0}{1}", Environment.NewLine, ex.Message);
                    GM.ReportError(this, ex, s);
                }
                finally
                {
                    Cursor = Cursors.Default;
